...
SET @car11 = UUID_TO_BIN(UUID()); SET @car12 = UUID_TO_BIN(UUID()); SET @car13 = UUID_TO_BIN(UUID()); SET @car14 = UUID_TO_BIN(UUID()); SET @car15 = UUID_TO_BIN(UUID());
SET @car16 = UUID_TO_BIN(UUID()); SET @car17 = UUID_TO_BIN(UUID()); SET @car18 = UUID_TO_BIN(UUID()); SET @car19 = UUID_TO_BIN(UUID()); SET @car20 = UUID_TO_BIN(UUID());

SET @car21 = UUID_TO_BIN(UUID());
...
(@pat01, @nit_unirio, 'EXTRATO MARINHO CONTRA HIV-1', 'PI 1012416-0', 'Extrato de produtos marinhos com ação inibitória da replicação do vírus HIV-1.', 'Biotecnologia', 1500000.00, 'Izabel Christina Paixão', 'DISPONIVEL'),
...
(@pat03, @nit_unirio, 'SISTEMA DE PRODUÇÃO SONORA', 'BR 10 2016 028373', 'Sistema portátil útil no auxílio da educação musical.', 'Tecnologia Assistiva', 120000.00, 'Mariano Gomes Pimentel', 'DISPONIVEL'),
(@pat04, @nit_unirio, 'PREBIÓTICO POTENCIALIZADO', 'BR 10 2018 002657', 'Produto capaz de potencializar o efeito probiótico.', 'Nutracêuticos', 890000.00, 'Cristina Yoshie Takeiti', 'DISPONIVEL'),
(@pat05, @nit_unirio, 'FILMES DE CAQUI', 'BR 10 2019 025723', 'Filme biodegradável a base de caqui para alimentos.', 'Sustentabilidade', 320000.00, 'Julia Rabelo Vaz Matheus', 'DISPONIVEL'),
...
(@pat07, @nit_unirio, 'PROTETOR DE TRAQUEOSTOMIA', 'BR 20 2020 004690', 'Dispositivo de baixo custo para higiene pessoal.', 'Saúde', 75000.00, 'Danielle Galdino De Paula', 'DISPONIVEL'),
...
(@pat09, @nit_unirio,'MICROSCÓPIO RECICLADO', 'BR 20 2023 007137', 'Microscópio digital produzido com material reciclável.', 'Educação', 45000.00, 'Guilherme De Souza Silva', 'DISPONIVEL'),
(@pat10, @nit_unirio, 'FARINHAS DE PINHÃO', 'BR 10 2024 016408', 'Metodologia para obtenção de farinhas nutritivas.', 'Agronegócio', 185000.00, 'Tatiane Veiga Gravino', 'DISPONIVEL');
